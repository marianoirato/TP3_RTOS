...
//OS_CPU_SR cpu_sr;                  // Lo tuve que agregar por un error
...
#define  OS_ENTER_CRITICAL() CPU_IntDis(); // do { cpu_sr = OS_CPU_SR_Save(CPU_CFG_KA_IPL_BOUNDARY << (8u - CPU_CFG_NVIC_PRIO_BITS));} while (0)
                                                 /* Restore CPU BASEPRI priority level.                */
#define  OS_EXIT_CRITICAL() CPU_IntEn();//  do { OS_CPU_SR_Restore(cpu_sr);} while (0)
#endif
